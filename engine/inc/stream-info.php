...
$listStream = $db->super_query("SELECT * FROM ".PREFIX."_streams ORDER BY id", true);
		...
		<tr>
			...
			<td>
				<table width="420px" style="margin-left:100px;">
					<tbody>
						<tr>
							<td width="140px" style="padding:3px;"><b style="color:red;">Разработку забросил</b></td>
...
===========================*/
	...
	if($dbquery) {
		...
		streamMsg("info", "Готово", "Трансляция успешно добавлена.<br /><br /><a href=$PHP_SELF?mod=stream-info&action=edit>$lang[db_prev]</a>");
	} else {
		$dle_api->clean_cache("stream-info");
		$dle_api->clean_cache("stream-info-block");
...
if(isset($id) && !empty($id)) {
		$stream = $db->super_query("SELECT * FROM ".PREFIX."_streams WHERE id = '".$id."'");
		echomenu();
		opentable();
