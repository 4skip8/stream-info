		...
		<tr>
			<td>
				<table width="100%" id="newslist">
					<tbody>
						<tr class="thead">
							<td style="padding-left: 7px;">Дата добавления</td>
							<td>Заголовок</td>
							<td width="210">Автор</td>
							<td width="120">Сервис</td>
							<!-- <td width="120">Транслирует?</td> -->
HTML;
						if ($_REQUEST['action'] == 'edit'){
						echo '<td width="10" style="text-align: center;"><input id="all" name="all" value="" type="checkbox"></td>';
						}
...
HTML;
	foreach($streams as $stream) {
		...
		echo<<<HTML
			<tr class="">
				<td class="list">
				<a style="margin-left: 7px;" class="list">{$stream['date']}</a>
				</td>
				<td class="list" style="padding:4px;">
					<a title="Редактировать {$stream['title']}" class="list" href="{$PHP_SELF}?mod=stream-info&action=edit&id={$stream['id']}">{$stream['title']}</a>
				</td>
				<td class="list">
...
function getstreaminfo($login) {
	...
	$db = new db();
	$stream_list = $db->super_query("SELECT title, service, login, description, pic, date FROM " . PREFIX . "_streams WHERE login = '".$login."'", true);
