...
Для указания ссылки на страницу трансляции, используйте тег ``{full_stream}``. Пример: ``<a href="{full_stream}">{title}</a>``
...
**Для вывода на главную страницу, вам нужно теги ``[stream-info]{stream-info}[/stream-info]`` вставить в любом месте main.tpl**.

#TODO
* Стриминг http://sc2tv.ru
* Чат для трансляций
